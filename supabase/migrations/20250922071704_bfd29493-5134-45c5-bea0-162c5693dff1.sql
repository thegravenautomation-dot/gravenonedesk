-- Enhanced Dispatch Module with AWB Management and Tracking
...
CREATE POLICY "Admin and dispatch can manage courier configs" 
...
USING (
  EXISTS (
    ...
    AND profiles.role IN ('admin', 'manager', 'dispatch')
  )
);
...
CREATE POLICY "Dispatch can upload AWB documents" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
  bucket_id = 'awb-documents' AND
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE id = auth.uid() 
    AND role IN ('admin', 'manager', 'dispatch')
  )
);
...
WITH CHECK (
  ...
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE id = auth.uid() 
    AND role IN ('admin', 'manager', 'dispatch', 'sales')
  )
);